...
#### 3.1. “하위 타입 객체를 반환할 수 있다”는 말의 의미

> 프레임워크 사용자에게는 인터페이스만 공개하면서, 내부적으로는 서로 다른 다양한 구현체를 마음대로 선택·교체할 수 있는 자유를 준다.
...
구현체를 숨긴다는 것은 캡슐화의 완성이다.
캡슐화는 단순히 변수를 숨기는것 뿐 아닌 구현체를 숨기는 행위도 포함한 단어이다.
내부의 값 뿐만 아니라 내부 구현을 외부에 노출시키지 않는다는 것이 캡슐화의 포괄적인 의미.
...
class AnimalFactory {
    // 구현체를 감추고 반환 타입은 `Animal` 인터페이스로 정의한다.
    static Animal newDog() {
        // new Dog는 어떤 Dog로도 대체할 수 있다.
        // 코드를 변경해도 클라이언트는 newDog을 사용해서 Animal을 반환받는데 전혀 지장이 생기지 않는다.
        return new Dog();   // Animal의 하위 타입 반환
    }

    static Animal newCat() {
        return new Cat();   // Animal의 하위 타입 반환
    }
}

```

이와같이 정의하면 `newDog` 라는 이름으로 redis 캐시를 생성한다.
단, 내부에서 사용되는 구현은 언제든 바뀌어도 `Animal`클래스르 생성한다는 사실은 변하지 않으며, 강아지를 생성할 것이라는 계약은 메서드 명을 통해 여전히 유효하다.

> 프레임워크 사용자에게는 인터페이스만 공개하면서, 내부적으로는 서로 다른 다양한 구현체를 마음대로 선택·교체할 수 있는 자유를 준다.

#### 3.2. 구현체를 숨길 수 있다 (캡슐화의 완성)

클라이언트는 오직 인터페이스만 보기때문에 Dog가 어떻게 생겼는지 알필요가 없다.

```java
Animal a = AnimalFactory.newDog();
```

외부 구현체인 Dog를 공개하지 않아도 된다.

#### 3.3. 구현체 교체가 가능해진다 (확장성과 진화를 가능하게 함)

조건에 따라 다른 하위 타입을 반환할 수 있다.

```java
static Animal new Dog(){
    if(fastMode) {
        return new HighPerformanceDog();
    } else {
...
또한 내부에서 마음대로 변경하더라도 클라이언트는 전혀 영향을 받지 않는다.
버그 수정이 자유롭고 진화도 가능하다.

이 능력은 생성자가 제공하지 못하는 능력이다.

정적팩터리를 사용하면 다형적 생성(Polymorphic Construction)이 가능하다.

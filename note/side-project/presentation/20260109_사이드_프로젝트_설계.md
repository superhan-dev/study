...
데이터의 특징은 서비스를 사용하기 힘들 배송이 과도하게 지연되는 경우가 있었고 개선의 여지가 있었다. 따라서 셀러들이 주문을 처리하는 속도와 전체 배송기간 등을 파악하여 보다 나은 서비스 구축을 위해 인사이트를 줄 수 있는 데이터와 기능을 구현하기에 적합하다고 판단했다.
...
3. 고객 만족도는 배송 속도·예측 정확도와 강한 상관관계가 있는지 식별한다.
...
- 상품별·셀러별·지역별 배송 이력을 분석하여 "어디서 → 어디로 → 얼마나 빨리" 배송되었는지를 루트 단위로 학습
- OpenStreetMap dataset을 활용하여 실제 도로와 경로의 가중치 연산함으로써 보다 짧은 시간이 드는 경로를 선정
...
이 시스템의 핵심은 **"문제가 터지기 전에 흐름의 이상을 감지하는 것"**이다.

#### 1. 발송 전 지연 (Pre-shipment Delay)셀러의 작업 구간에서 발생하는 병목을 감지합니다.

- 비교 대상: 해당 셀러의 과거 평균 처리 시간 vs 카테고리 내 상위 셀러 평균 처리 시간
- 핵심 로직 (Threshold-based): \* 임계값(Threshold)
- 설정: 단순 '24시간'이 아니라, 해당 카테고리(예: 신선식품 vs 가구)의 상위 80% 셀러의 평균 처리 시간을 기준으로 설정.
- 경보 시스템: 결제 완료 후 $T+N$ 시간이 경과했음에도 '배송사 인계(Wait for Pickup)' 상태로 전환되지 않을 경우 대시보드에 '지연 위험' 표시.
- 개입 액션:
  - 자동 알림: 셀러에게 "현재 주문량이 많아 발송 지연이 예상됩니다. 송장 출력을 확인해주세요." 메시지 발송.
  - 운영자 개입: 동일 지역 셀러 집단에서 공통 지연 발생 시, 해당 지역 택배사 허브의 이슈인지 확인.

#### 2. 배송 중 지연 (In-transit Delay)택배사 인계 이후 고객에게 도착하기까지의 구간 모니터링.

- 비교 대상: 동일 노선(출발지-목적지 ZIP code), 동일 택배사, 동일 시간대 이동 중인 다른 물동량의 이동 속도.
- 핵심 로직 (Anomaly Detection):
  - 분포 기반 이상치 탐지: carrier_date부터 customer_date까지의 소요 시간을 실시간 분포로 시각화합니다.
  - 이상 징후 포착: 특정 허브(Hub)나 터미널(Sub-terminal)에 머무는 시간이 전체 평균의 $+1.5\sigma$(표준편차)를 벗어날 경우 '흐름 정체'로 판단.
  - 개입 액션:
    - 고객 선제 안내: "고객님의 상품이 현재 OO 터미널에 머물고 있습니다. 폭설로 인해 평소보다 1일 더 소요될 예정이니 양해 부탁드립니다." (고객 문의 전 선제 대응)
    - 배송 수단 변경: 극심한 지연이 예상되는 구간은 후속 물량에 대해 즉시 다른 배송사로 우회하도록 설정.

#### 3. 데이터 분석 프레임워크 (Technical Approach)시스템이 학습해야 할 데이터 포인트.

| 구분             | 데이터 소스 (Feature)                      | 분석 방법                                    |
| ---------------- | ------------------------------------------ | -------------------------------------------- |
| 셀러 퍼포먼스    | 주문 접수 ~ 송장 출력 시간, 패킹 시간      | 하위 10% 셀러 식별 및 개선 가이드 제공       |
| 구간별 소요 시간 | 터미널 간 이동 시간, 라스트 마일 배송 시간 | 구간별 병목 현상(Bottleneck) 실시간 모니터링 |
| 외부 변수        | 날씨, 공휴일, 이벤트(빅세일 기간)          | 가중치 적용 (Wweather​, Wseason​)            |

#### 4. 기대 효과 (Business Impact)

- CS 비용 절감: 고객이 "내 택배 왜 안 와요?"라고 묻기 전에 먼저 정보를 제공함으로써 단순 문의 30% 이상 감소.
- 셀러 관리 고도화: 상습 지연 셀러를 조기에 발견하여 패널티 부여 또는 운영 컨설팅 제공.
- 브랜드 신뢰도: 지연이 발생하더라도 "시스템이 나를 챙기고 있다"는 느낌을 주어 구매 경험 방어.
